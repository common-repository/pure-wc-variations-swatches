...
class Tp_Wvs {
	...
	public function __construct() {
		...
		$this->plugin_name = 'pure-wc-variation-swatches';
	...
	private function load_dependencies() {
		 ...
		 * core plugin.
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'includes/class-pure-wc-swatches-loader.php';

		/**
		 * The class responsible for defining internationalization functionality
		 * of the plugin.
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'includes/class-pure-wc-swatches-i18n.php';

		/**
		 * The class responsible for defining all actions that occur in the admin area.
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'admin/class-pure-wc-swatches-admin.php';

		/**
		 * The class responsible for defining all actions that occur in the public-facing
		 * side of the site.
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'public/class-pure-wc-swatches-public.php';
	...
	private function define_public_hooks() {
		...
		if($get_options['enable_swatches'] == true){
			$position = $get_options['swatch_position'];

			if ( ! empty( $position ) ) {
				switch ( $position ) {
					case 'after_thumbnail':
						$this->loader->add_action( 'woocommerce_shop_loop_item_title', $plugin_public, 'variation_attribute_html_shop_page',  9 );
						break;
					case 'after_title':
						$this->loader->add_action( 'woocommerce_shop_loop_item_title', $plugin_public, 'variation_attribute_html_shop_page',  11 );
						break;
					case 'after_rating':
						$this->loader->add_action( 'woocommerce_after_shop_loop_item_title', $plugin_public, 'variation_attribute_html_shop_page',  6 );
						break;
					case 'after_price':
						$this->loader->add_action( 'woocommerce_after_shop_loop_item_title', $plugin_public, 'variation_attribute_html_shop_page',  11 );
						break;
					case 'after_add_to_cart':
						$this->loader->add_action( 'woocommerce_after_shop_loop_item', $plugin_public, 'variation_attribute_html_shop_page',  11 );
						break;
					default:
						$this->loader->add_action( 'woocommerce_after_shop_loop_item_title', $plugin_public, 'variation_attribute_html_shop_page',  10 );
				}
			}
